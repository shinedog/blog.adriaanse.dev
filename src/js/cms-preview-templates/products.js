...
const ProductsPreview = ({entry, getAsset}) => {
  ...
  const blurbs = entry.getIn(["data", "intro", "blurbs"]) || [];
  const testimonials = entry.getIn(["data", "testimonials"]) || [];
  const plans = entry.getIn(["data", "pricing", "plans"]) || [];
      ...
      <div className="mw7 center ph3 pv4">
        <div className="flex flex-wrap mhn1">
          <div className="w-100 w-50-ns ph1-ns">
            <img src={getAsset(entry.getIn(["data", "main", "image1", "image"]))} alt="" />
          </div>
          <div className="w-100 w-50-ns ph1-ns">
            <img src={getAsset(entry.getIn(["data", "main", "image2", "image"]))} alt="" />
          </div>
          <div className="w-100 ph1-ns">
            <img src={getAsset(entry.getIn(["data", "main", "image3", "image"]))} alt="" />
          </div>
        </div>
      </div>
        ...
        {testimonials.map((testimonial, index) => (
          <div className="center mb3 ph3" key={index}>
            <blockquote className="bg-grey-1 primary pa3 mb3 br1 b mw6 center">
              <p className="f4 mb0">&ldquo;{testimonial.get("quote")}&rdquo;</p>
            ...
            {plans.map((plan, index) => (
              <div className="w-33-ns ph2" key={index}>
                <div className="ph2">
                  ...
                  <ul>
                    {(plan.get("items") || []).map((item, i) => (
                      <li key={i}>
                        <p className={i + 1 !== plan.get("items").size ? "pb2 mb2 divider-grey" : null}>{item}</p>
                      </li>
                    ))}
                  </ul>
                </div>
